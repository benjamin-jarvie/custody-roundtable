...
with better UX and more familiarity. Closing it does not mean outsourcing
...
1. Generate your own seed material. This takes about ten minutes with a deck
   of playing cards.
...
count every step a bank account actually requires. The perceived
difficulty is what is out of proportion.
...
- **Non-deterministic output** (an actual random draw) cannot be replicated
  or proven after the fact. It can only be sourced by you in the first place.
...
different software can derive a wallet that looks completely valid, with a
correct checksum and correct format, and shows a zero balance, because it derived the
...
The biggest risk in this setup is losing both seeds, so make copies as needed.
...
whether to add redundancy, because most people already have some. It is
whether to do it deliberately, with a scheme built for the purpose.
